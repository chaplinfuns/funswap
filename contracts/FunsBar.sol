...
contract FunsBar is ERC20("FunsBar", "iFUNS") {
    ...
    function enter(uint256 _amount) public {
        ...
        if (totalShares == 0 || totalfuns == 0) {
            mint(msg.sender, _amount);
        } else {
            uint256 what = _amount.mul(totalShares).div(totalfuns);
    ...
    function leave(uint256 _share) public {
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(funs.balanceOf(address(this))).div(totalShares);
    ...
    function getFunsShare(address _user) public view returns (uint256 funsAmt) {
        return balanceOf(_user).mul(funs.balanceOf(address(this))).div(totalSupply());
    }
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FUNS (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FUNS::_writeCheckpoint: block number exceeds 32 bits");
